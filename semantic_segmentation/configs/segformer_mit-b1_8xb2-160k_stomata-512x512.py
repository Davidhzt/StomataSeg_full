# Segformer + MIT-B1 configuration for Stomata Dataset
...
model = dict(
    ...
    backbone=dict(
        ...
        in_channels=3,
        embed_dims=64,  # MIT-B1 uses 64 instead of 32
        num_stages=4,
        num_layers=[2, 2, 2, 2],
        num_heads=[1, 2, 5, 8],
        patch_sizes=[7, 3, 3, 3],
    ...
    decode_head=dict(
        type='SegformerHead',
        in_channels=[64, 128, 320, 512],  # MIT-B1 uses different channel sizes
        in_index=[0, 1, 2, 3],
        channels=256,
        dropout_ratio=0.1,
...
test_evaluator = dict(type='IoUMetric', iou_metrics=['mIoU'], output_dir='test_results', keep_results=True)
...
test_cfg = dict(type='TestLoop') # Top-level test_cfg
...
work_dir = './work_dirs/segformer_mit-b1_8xb2-160k_stomata-512x512'
